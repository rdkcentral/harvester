...
log "INFO" "Running make for harvester_gtest.bin..."
...
if [ -f "./source/test/harvester_gtest.bin" ]; then
    log "INFO" "Running harvester_gtest.bin"
    ./source/test/harvester_gtest.bin
    log "INFO" "Completed Test Execution"
else
    log "ERROR" "harvester_gtest.bin does not exist, cannot run tests"
    exit 1
fi
