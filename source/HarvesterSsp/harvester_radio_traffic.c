...
int getRadioBssid(int radioIndex, char* radio_BSSID)
{
    ...
    if (ret)
    {
       CcspHarvesterTrace(("RDK_LOG_ERROR, Harvester %s : wifi_getSSIDMACAddress returned error [%d] \n",__FUNCTION__ , ret));
        ...
        {
#ifdef CORE_NET_LIB
            libnet_status result_mac = CNL_STATUS_FAILURE;
            result_mac = interface_get_mac(radioIfName, radio_BSSID, 19);
            if (result_mac == CNL_STATUS_SUCCESS)
            {
                for (char *p = radio_BSSID; *p; p++)
                {
                    *p = toupper((unsigned char)*p);
                }
                CcspHarvesterTrace(("RDK_LOG_INFO, MAC address fetched for Interface [%s] via corenet api: [%s] \n", radioIfName, (radio_BSSID[0] == '\0') ? "NULL" : radio_BSSID));
            }
            else
            {
                CcspHarvesterTrace(("RDK_LOG_ERROR, Harvester %s : Failed to get MAC address via CoreNetLib api \n", __FUNCTION__));
            }
#else
            FILE *f = NULL;
...
void* StartRadioTrafficHarvesting( void *arg )
{
    ...
    /* Polling should continue whenever RadioInterfaceStatistics.Enabled is set to true. */
    while (GetRISHarvestingStatus()) {
#else
